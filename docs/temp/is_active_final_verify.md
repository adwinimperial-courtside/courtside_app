261:  const activePlayers     = existingStats.filter(s => s.is_active);
264:  const homeActiveCount = existingStats.filter(s => s.team_id === game?.home_team_id && s.is_active).length;
265:  const awayActiveCount = existingStats.filter(s => s.team_id === game?.away_team_id && s.is_active).length;
321:      const activeIds = new Set(freshStats.filter(s => s.team_id === teamId && s.is_active).map(s => s.player_id));
341:      const activeIds  = new Set(freshStats.filter(s => s.team_id === teamId && s.is_active).map(s => s.player_id));
370:  // is_active = currently on court. is_starter = began game in starting 5.
